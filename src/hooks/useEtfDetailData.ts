      ...
      try {
        ...
        const [dailyRes, indicatorsRes, signalsRes, fitRes, momentumRes] = await Promise.all([
          ...
          supabase
            ...
            .select('symbol,trade_date,name,close,action,action_type,buy_signals,sell_signals,buy_count,sell_count,signal_score,event_buy_count,event_sell_count,event_buy_signals,event_sell_signals,state_buy_count,state_sell_count,state_buy_signals,state_sell_signals,k,d,j,rsi,macd_hist')
            .eq('symbol', symbol)
            .gte('trade_date', sixMonthsAgo)
            .order('trade_date', { ascending: false }),
          supabase
